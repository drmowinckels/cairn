...
// idle / about / notify popups. WKWebView renders a transparent root as
// blank on a non-transparent (decorated) macOS window, so the main window
// must be opaque.
try {
  const win = new URLSearchParams(window.location.search).get("win");
  if (win === "idle" || win === "about" || win === "notify") {
    document.documentElement.dataset.win = win;
  } else {

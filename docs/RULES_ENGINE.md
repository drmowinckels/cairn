...
| `app.category`   | Derived in-matcher from `app.name` via the bundled `app_categories.json` table           | Coarse category of the frontmost app (`meeting` · `editor` · `terminal` · `browser`) — one rule covers Zoom/Teams/Webex without naming each |
...
pub enum SignalKey {
    AppName, AppCategory, WindowTitle, IdeFolder, GitBranch, GitRepo,

...
- Implemented via a tiny browser **extension** that pushes the active-tab URL/domain/title to a local Unix-domain socket (or named pipe on Windows) under the app data dir (`<data_dir>/ipc/sock`). The extension is open-source and ships separately; the Safari/Firefox/Chrome wrappers are tracked separately from the in-app receiver.
